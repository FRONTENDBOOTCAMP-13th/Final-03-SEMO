  ...
  useEffect(() => {
    async function fetchData() {
      if (isCurrentUser && user?.token?.accessToken) {
        try {
          const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/orders/`, { headers: { 'Client-Id': process.env.NEXT_PUBLIC_CLIENT_ID || '', Authorization: `Bearer ${user.token.accessToken}` } });
          if (res.ok) setPurchaseCount((await res.json()).pagination.total);
        } catch (error) { console.error("Failed to fetch user purchases:", error); }
      }
    }
    fetchData();
    ...
    <div className="w-full bg-uni-white text-uni-black font-sans">
      <header className="bg-uni-white px-3 py-2 sticky top-0 z-10">
        <div className="flex items-center justify-between h-11">
          <button onClick={() => router.back()} className="p-2 -ml-2 flex items-center justify-center"><ChevronLeft className="w-6 h-6 text-uni-gray-600" /></button>
          <h2 className="absolute left-1/2 -translate-x-1/2 text-18 font-semibold text-uni-black font-pretendard">판매자 프로필</h2>
