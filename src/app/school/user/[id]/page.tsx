...
import { Star, Heart, MessageCircleMore, ChevronLeft } from "lucide-react";
...
export default function UserProfilePage() {
  ...
  // 사용자 게시물 가져오기 (my-post와 동일한 방식 - sell, buy, groupPurchase)
  const fetchUserPosts = async (userId: string) => {
    try {
      console.log("fetchUserPosts 호출:", userId);

      // 현재 사용자인 경우 my-post와 동일한 방식으로 가져오기
      if (isCurrentUser && user?.token?.accessToken) {
        console.log("현재 사용자 - 인증된 API 호출");

        const [sellRes, buyRes, groupPurchaseRes] = await Promise.all([
          fetch(`${process.env.NEXT_PUBLIC_API_URL}/posts/users/?type=sell`, {
            headers: {
              "Client-Id": process.env.NEXT_PUBLIC_CLIENT_ID || "",
              Authorization: `Bearer ${user.token.accessToken}`,
            },
          }),
          fetch(`${process.env.NEXT_PUBLIC_API_URL}/posts/users/?type=buy`, {
            headers: {
              "Client-Id": process.env.NEXT_PUBLIC_CLIENT_ID || "",
              Authorization: `Bearer ${user.token.accessToken}`,
            },
          }),
          fetch(`${process.env.NEXT_PUBLIC_API_URL}/posts/users/?type=groupPurchase`, {
            headers: {
              "Client-Id": process.env.NEXT_PUBLIC_CLIENT_ID || "",
              Authorization: `Bearer ${user.token.accessToken}`,
            },
        ...
        if (sellRes.ok) {
          ...
          if (sellData.ok && sellData.item) {
            console.log("받은 sell 게시물 개수:", sellData.item.length);
            console.log("첫 번째 sell 게시물 전체 데이터:", sellData.item[0]);
            console.log("첫 번째 sell 게시물 댓글 수:", sellData.item[0]?.repliesCount);
            setSellPosts(sellData.item);
          }
        }

        if (buyRes.ok) {
          const buyData = await buyRes.json();
          if (buyData.ok && buyData.item) {
            console.log("받은 buy 게시물 개수:", buyData.item.length);
            setBuyPosts(buyData.item);
          }
        }

        if (groupPurchaseRes.ok) {
          const groupData = await groupPurchaseRes.json();
          if (groupData.ok && groupData.item) {
            console.log("받은 groupPurchase 게시물 개수:", groupData.item.length);
            setGroupPurchasePosts(groupData.item);
          }
        }
      } else {
        // 다른 사용자의 경우 일반 API 호출 (타입별로)
        console.log("다른 사용자 - 일반 API 호출");

        const [sellRes, buyRes, groupPurchaseRes] = await Promise.all([
          fetch(`${process.env.NEXT_PUBLIC_API_URL}/posts/users/${userId}?type=sell`, {
            headers: {
              "Client-Id": process.env.NEXT_PUBLIC_CLIENT_ID || "",
            },
          }),
          fetch(`${process.env.NEXT_PUBLIC_API_URL}/posts/users/${userId}?type=buy`, {
            headers: {
              "Client-Id": process.env.NEXT_PUBLIC_CLIENT_ID || "",
            },
          }),
          fetch(`${process.env.NEXT_PUBLIC_API_URL}/posts/users/${userId}?type=groupPurchase`, {
            headers: {
              "Client-Id": process.env.NEXT_PUBLIC_CLIENT_ID || "",
            },
          }),
  ...
  // 사용자 구매 정보 가져오기 (현재 사용자인 경우에만)
  const fetchUserPurchases = async () => {
    if (!isCurrentUser || !user?.token?.accessToken) {
      setPurchaseCount(0); // 다른 사용자의 구매 정보는 볼 수 없음
      return;
    }
  ...
  const fetchReviewsData = async () => {
    ...
    try {
      ...
      if (reviewsResponse.ok && reviewsResponse.item) {
        ...
        reviewsResponse.item.forEach((product) => {
          product.replies.forEach((reply) => {
            allRatings.push(reply.rating);
          });
        });
  ...
  // 이미지 경로 처리 함수 (ProductCard와 동일한 방식 사용)
  const getProductImagePath = (productImages: any) => {
    // productImages가 배열인 경우
    ...
    // productImages가 문자열인 경우
    if (typeof productImages === "string") {
      return productImages;
    }

    // 기본값 반환 (null이면 ProductCard의 getImageUrl에서 기본 이미지 처리)
    return "";
  };
