...
//       // 개인룸에서 내가 보낸 메시지인 경우 서버 응답 무시
//       if (currentRoomId !== GLOBAL_ROOM_ID && !isWhisper) {
//         // 서버에서 받은 메시지가 내가 보낸 것인지 확인
//         // user_id로 확인 (서버에서 user_id를 제대로 보내는 경우)
//         if (raw.user_id && String(raw.user_id) === String(userId)) return;

//         // 서버에서 user_id가 없는 경우, 최근 메시지와 비교하여 중복 체크
